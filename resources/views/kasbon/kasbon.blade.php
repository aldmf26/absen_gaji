...
@section('content')
                    ...
                    @else
                        <div class="card-header">
                            <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#tambah"><i
                                    class="fa fa-plus"></i> Tambah kasbon</button>
                        </div>
                    @endif
                    <div class="card-body">
                                            ...
                                            @else
                                                <td>
                                                    <a href="#" class="btn btn-sm btn-warning"
                                                        data-target="#edit{{ $k->id_kasbon }}" data-toggle="modal"><i
                                                            class="fa fa-edit"></i></a>
                                                    <a onclick="return confirm('Apakah yakin dihapus ?')"
                                                        href="{{ route('hapusKasbon', ['id_kasbon' => $k->id_kasbon]) }}"
    ...
    <!-- Modal -->
    <form action="{{ route('tambahKasbon') }}" method="post">
        @csrf
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content modal-md">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Tambah Kasbon</h5>
    ...
    {{-- modal edit --}}
    @foreach ($kasbon as $k)
        <form action="{{ route('editKasbon') }}" method="post">
            @csrf
            <div class="modal fade" id="edit{{ $k->id_kasbon }}" tabindex="-1" role="dialog"
                aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content modal-md">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Edit Kason</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
